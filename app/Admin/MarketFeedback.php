...
{
    //
    protected $table = 'market_feedbacks';
    protected $fillable = [
        'user_id',
        'processing_date',
        'issue',
    ...
    public function getIssue()
    {
        $issue_id = $this->issue;
        if (!empty($issue_id)) {
            return (!empty(config('myconfig.issue_market_feedback_agent')[$issue_id])) ? config('myconfig.issue_market_feedback_agent')[$issue_id] : '';
        }
        return '';
    }
    ...
    public static function getMarketFeedbackSale($request){
        ...
        $marketFeedbacks = MarketFeedback::when($request->get('agent_market_feedback_filter'), function ($query) use ($request) {
        ...
        return $marketFeedbacks->orderBy('processing_date', 'desc');
    }
}
