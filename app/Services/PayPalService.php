...
use function MicrosoftAzure\Storage\Samples\queryEntitiesSample;
...
//                 ->setSku($data['sku']) // ID của item
    ...
    public function createPaymentCredit($transactionDescription)
    {
        $checkoutUrl = false;
        ...
        $url_component = parse_url($checkoutUrl);
        parse_str($url_component['query'], $params);
        $checkoutUrl = $url_component['scheme'] . '://' . $url_component['host'] . '/webapps/xoonboarding?token=' . $params['token'];
        return $checkoutUrl;
    }
