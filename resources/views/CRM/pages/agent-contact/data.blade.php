@can('agentContact.index')
    @foreach($agentContactDatas as $data)
        <tr id="agent_contact_data_{{$data->id}}">
            ...
            <td class="white-space-break-spaces" style="overflow: hidden;text-overflow: ellipsis;">{{!empty($data->name) ? $data->agent_name:''}}</td>
