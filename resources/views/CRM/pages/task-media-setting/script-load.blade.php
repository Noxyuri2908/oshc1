<script>
    ...
    $(document).on('click', '.{{$elementClassSubmitForm}}', function (e) {
        ...
        @endforeach
            categorys = [];
        $("#modal_media_setting_form input[name='value[]']").each(function() {
        ...
        ,
            error: function (err) {
                ...
                if (err.status == 422) {
                    ...
                    @endforeach
                    $("#modal_media_setting_form input[name='value[]']").each(function(i,obj) {
                        categorys.push($(this).val());
                        var categoryName = 'category.'+i;
                        if (categoryName in err.responseJSON.errors) {
                            $('#category'+i+'_div_alert').text(err.responseJSON.errors[categoryName][0]);
                        }
                    });
                }
            }
