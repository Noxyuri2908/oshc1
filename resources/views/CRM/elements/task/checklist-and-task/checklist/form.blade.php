...
{{--                                           name="level_of_process" id="level_of_process{{$type}}" type="number" required>--}}
            ...
            <div class="modal-footer">
                <button class="btn btn-success mr-1 mb-1 btn_submit_{{$type}}_form" type="submit" is-click="false"
                        data-url="{{!empty($checkListData)?route('check-list.update',['id'=>$checkListData->id,'group_id'=>$groupId]):route('check-list.store',['group_id'=>$groupId])}}">{{!empty($checkListData)?'Update':'Submit'}}</button>
