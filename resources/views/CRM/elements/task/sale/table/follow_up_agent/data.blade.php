...
@foreach($followUps as $item)
    <tr id="follow-ups-{{$item->id}}">
        ...
        <td class="white-space-break-spaces text-overflow color-white {{setLabelFlUpStatus($item->follow_up_status)}} text-center font-bold" style="white-space: pre-line">{{getValueByIndexConfig($fl_up_status, $item->follow_up_status)}}</td>
        <td class="white-space-break-spaces text-overflow text-center" style="white-space: pre-line">@if($item->hot_issue == 1)<i class="fas fa-check"></i>@endif</td>
        ...
        <td class="white-space-break-spaces text-overflow {{setLabelStatus($item->status)}} text-center font-bold">{{$item->getStatus()}}</td>
