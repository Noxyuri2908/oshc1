@foreach($data as $tmp)
    <tr class="data-customer" id="data-customer_{{$tmp->id}}" is-render='false' data-id="{{$tmp->id}}">
                    ...
                    <div class="bg-white py-2">
                        @can('customer.edit')
                            <a class="dropdown-item customer_data_edit" data-id="{{$tmp->id}}" style="cursor: pointer"
                               href="{{route('customer.edit',['id'=>$tmp->id,'page'=>request()->get('page')])}}"
                               data-url_edit="{{route('ajax.customer.showData',['tab'=>'cus','id'=>$tmp->id])}}">Edit</a>
                        @endcan
        ...
        <td class="align-middle">{{!empty($tmp->customers->first()) ? getCounsellorById($tmp->customers->first()->person_counsellor_id) : ''}}</td>
        ...
        <td class="align-middle" id="staff_id">{{$tmp->staff != null ? $tmp->staff->admin_id : ''}}</td>
        <td class="align-middle text-overflow">
            <a href="#" data-toggle="modal"
               data-target="#note_data{{$tmp->id}}">{{decode_html($tmp->note, 'customer')}}</a>
            <div class="modal fade" id="note_data{{$tmp->id}}" tabindex="-1" role="dialog"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div>
                                <p style="white-space: pre-wrap;">{{decode_html($tmp->note)}}</p>
                            </div>
                        </div>
                    </div>
                </div>
