    ...
    @if($agent != null && isset($hh) && isset($comm) && $hh != null && $comm != null && $providerCom != null && $refund != null && $profit != null)
        <tr class="data-refund" id="data-refund_{{$tmp->id}}" data-id="{{$invoice->id}}">
                ...
                <div class="dropdown text-sans-serif">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        @can('refundInvoice.edit')
                            <a class="dropdown-item refund_data_edit" data-id="{{$tmp->id}}"
                               data-url_edit="{{route('ajax.customer.showData',['tab'=>'refund','id'=>$tmp->id])}}"
            ...
            <td style="background-color: #bfffff"
                title="Profit 1">{{convert_price_float($profit->profit_extra_money)}}</td>
            <td style="background-color: #bfffff" title="Profit 1">{{convert_price_float($profit->profit_total)}}</td>
            <td style="background-color: #bfffff"
                title="Profit 1">{{convert_price_float($profit->profit_exchange_rate)}}</td>
            <td style="background-color: #bfffff"
                title="Profit 1">{{convert_price_float($profit->profit_money_VND)}}</td>
            <td style="background-color: #bfffff"
                title="Profit 1">{{convert_price_float($profit->profit_bankfee_VND)}}</td>
            <td style="background-color: #bfffff" title="Profit 1">{{convert_price_float($profit->gst)}}</td>
            <td style="background-color: #bfffff"
                title="Profit 1">{{$profit->profit_status == 1 ? 'Done' : ($profit->profit_status == 2 ? 'Refund' : '')}}</td>
            <td style="background-color: #bfffff"
                title="Profit 1">{{$profit->comm_status == 1 ? 'Done' : ($profit->comm_status == 2 ? 'Refund' : '')}}</td>
            <!-- Revenue -->
            ...
            <td style="background-color: #bfbfff" title="Annalink received ">{{convert_price_float($difference)}}VNĐ
            </td>
            <!-- Annalink received -->
            ...
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{$profit->pay_agent_bonus}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{convert_price_float($profit->invoice->comm)}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{convert_price_float($profit->pay_agent_deduction)}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{convert_price_float($profit->pay_agent_total_amount)}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{convert_price_float($profit->pay_agent_exchange_rate)}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{convert_price_float($profit->vnd)}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{convert_price_float($profit->pay_agent_amount_VN)}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{Carbon::parse($profit->pay_agent_date)->format('d/m/Y')}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{$profit->gst_status_agent_profit == 1 ? 'Included' : (($profit->gst_status_agent_profit == 2) ? 'Not included ' : '')}}</td>
            <td style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">{{Carbon::parse($profit->issue_date_com_agent)->format('d/m/Y')}}</td>
            <td class="align-middle text-overflow" style="background-color: #fffe98"
                title="Pay commission for Agent/Cousellor">
                <a href="javascript:void(0)" data-toggle="modal"
                   data-target="#note_cp_{{$profit->id}}">{{$profit->note_cp}}</a>
                <div class="modal fade" id="note_cp_{{$profit->id}}" tabindex="-1" role="dialog"
            ...
            <td style="background-color: #ffbfff" title="Commission received from provider" class="text-overflow">
                <a href="" data-toggle="modal"
                   data-target="#note_of_re_{{$profit->id}}">{{$profit->note_of_receipt}}</a>
                <div class="modal fade" id="note_of_re_{{$profit->id}}" tabindex="-1" role="dialog"
                     aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
            ...
            <td>{{!empty($refund->refund_bank_pp) && !empty(getBank($refund->refund_bank_pp)) ? getBank($refund->refund_bank_pp)->account : ''}}</td>
            ...
            <td class="text-overflow">
                ...
                <div class="modal fade" id="std_note_{{$refund->id}}" tabindex="-1" role="dialog"
                     aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
            ...
            <td class="text-overflow">
                ...
                <div class="modal fade" id="note_2_{{$refund->id}}" tabindex="-1" role="dialog"
                     aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
