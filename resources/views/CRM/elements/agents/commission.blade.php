<div class="card mb-3">
    ...
    <div class="card-body bg-light" data-id="com">
        @can('commissionAgent.store')
            @if(empty($is_show))
                <a href="#" class="mb-4 d-block d-flex align-items-center add-comm-slide-show"><span class="circle-dashed">
            ...
            <form>
                ...
                <div class="form-group">
                    <div class="row">
                        ...
                        @foreach($services as $service)
                            <input type="hidden" id="info_service_{{$service->id}}" data-src="{{$service->image}}" data-name="{{$service->name}}">
                        @endforeach
                        <div class="col-lg-7">
                ...
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-lg-7">
                            <input class="form-control orm-control-sm datetimepicker" id="end_date" name="end_date" type="text" data-options='{"dateFormat":"d/m/Y"}'>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <div class="row">
                        <div class="col-lg-7 offset-lg-3">
                            @if(empty($is_show))
                                @can('commissionAgent.store')
                                    <button class="btn btn-primary add_new_comm" id="add_new_comm" type="button">Add</button>
                                @endcan
        ...
        @if(isset($obj))
            <div id="data-old">
                @foreach($comms as $comm)
                    <input type="hidden" id="old_comm_{{$comm->id}}" data-service="{{$comm->type_service}}" data-type="{{$comm->type}}" data-comm="{{$comm->comm}}" data-date="{{$comm->date}}">
                @endforeach
            </div>
            <div id="data-hidden">
                {{-- @foreach($comms as $comm)
