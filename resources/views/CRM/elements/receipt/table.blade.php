        ...
        @foreach($phieuthus as $tmp)
            ...
            @if($invoice != null)
                <tr class="edit_phieuthu" data-id="{{$tmp->id}}">
                    <td>
                        <div class="dropdown">
                            <button class=" btn btn-link dropdown-toggle" type="button" id="dropdownApplyReceipt{{$tmp->id}}"
                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <span class="fas fa-ellipsis-h fs--1"></span>
                            </button>
                            <div class="dropdown-menu" aria-labelledby="dropdownApplyReceipt{{$tmp->id}}">
                                <div class="bg-white py-2">
                                    <a class="dropdown-item edit-receipt-customer" data-toggle="modal" data-target="#myModalReceipt" data-id="{{$tmp->id}}" data-apply_id="{{(!empty($getApply))?$getApply->id:''}}" href="#">Edit</a>
                                    <a class="dropdown-item text-danger delete-receipt-customer" data-id="{{$tmp->id}}" data-apply_id="{{(!empty($getApply))?$getApply->id:''}}"  href="#!">Delete</a>
                                </div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td>{{convert_price_float(round($sum_amount/($invoice->net_amount + $sum_bank_fee), 2))}}</td>
