        ...
        <fieldset>
            ...
            <div class="form">
                ...
                <div class="form-group clearfix">
                    ...
                    <div class="input-contenr">
                        <input type="text" class="form-control" id="refund_amount_com_agent_gbcfa"
                               value="{{!empty($refund)?$refund->refund_amount_com_agent_gbcfa:0}}">
                    </div>
                </div>
                <div class="form-group clearfix">
                    ...
                    <div class="input-contenr">
                        <select name="" id="status" class="form-control">
                            @foreach(config('myconfig.status_refund_recall') as $key=>$value)
                                <option
                                    value="{{$key}}" {{$refund != null ? ($refund->status == $key ? 'selected' : '') : ''}}>{{$value}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group clearfix">
                    <label class="control-label">Date of recall</label>
                    <div class="input-contenr">
                        <input class="form-control choose-date-form" id="date_of_recall" type="text"
                               value="{{$refund != null ? convert_date_form_db($refund->date_of_recall) : ''}}">
                    </div>
                </div>
                <div class="form-group clearfix">
                    <label class="control-label">Note</label>
                    <div class="input-contenr">
                        <textarea name="" id="note2" cols="30" class="form-control"
                                  rows="10">{{$refund != null ? $refund->note2 : ''}}</textarea>
                    </div>
                </div>
